...
double CallZC::price_theory_CallZC(const vector<double> &term_structure, const vector<double> & initial_zero_coupon_){
   double a=mod_->a_;
   double Y=(1-exp(-a*(zc_maturity_-T_)))/a;
  ...
  // vector<double> initial_zero_coupon_=mod_->initial_zero_coupon(initial_forward_rate, term_structure);
   double sigmaP2=sigma*sigma*(1-exp(-2*a*T_))*Y*Y/(2*a);
   ...
   // find t, T position
	...
	for (size_t i = 0; i<term_structure_S.size(); i++) {
		term_structure_S[i] = abs(term_structure_S[i] - zc_maturity_);
	}
	...
	for (size_t i = 0; i<term_structure_T.size(); i++) {
		term_structure_T[i] = abs(term_structure_T[i] - T_);
	}
   ...
   double h=log(initial_zero_coupon_[index_S]/(initial_zero_coupon_[index_T]*strike_))/sigmaP+sigmaP/2.;
   double price=initial_zero_coupon_[index_S]*cdfGaussian(h)-strike_*initial_zero_coupon_[index_T]*cdfGaussian(h-sigmaP);
   return price;
}
